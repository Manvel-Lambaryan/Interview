...
GET `/urls?tag=:name` — filter all short URLs by tag name
...
GET `/urls/:short_code/analytics?from=DATE&to=DATE` — filter analytics by date range

---

## 🔗 How the Two Parts Connect

Mane's Click entity references Manvel's ShortURL via short_url_id . The click recording in Mane's part is triggered right after the redirect in Manvel's GET `/urls/:short_code` endpoint — they must coordinate this behavior.
